...
exports.getAllItems = async (req, res) => {
  try {
    ...
    res
      ...
      .json({ error: ERROR.FETCHING_ITEM });
  }
};
  ...
  try {
    ...
    if (!item) {
      return res
        ...
        .json({ error: ERROR.ITEM_NOT_FOUND });
    }
...
exports.addItem = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .json({ message: SUCCESS.ITEM_ADDED, item: newItem });
  } catch (error) {
...
exports.updateItem = async (req, res) => {
  ...
  try {
    const updatedItem = await Inventory.findOneAndUpdate({ prodid }, updates, {
      new: true,
    });
    if (!updatedItem) {
      return res
    ...
    res
      ...
      .json({ message: SUCCESS.ITEM_UPDATED, item: updatedItem });
  } catch (error) {
    res
      .status(STATUS_CODES.INTERNAL_ERROR)
      .json({ error: ERROR.UPDATING_ITEM });
  }
};
...
exports.deleteItem = async (req, res) => {
  ...
  try {
    ...
    res
      .status(STATUS_CODES.OK)
      .json({ message: SUCCESS.ITEM_DELETED, item: deletedItem });
  } catch (error) {
    res
      .status(STATUS_CODES.INTERNAL_ERROR)
      .json({ error: ERROR.DELETING_ITEM });
  }
};
